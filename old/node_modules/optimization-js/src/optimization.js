...
module.exports.minimize_Powell = function (fnc, x0) {
    ...
    while (!convergence) {
        ...
        var indices = shuffleIndicesOf(x);
        convergence = true;
        ...
        for (var i = 0; i < indices.length; i++) {
            ...
            x[indices[i]] = x[indices[i]] - alpha * dx;
            fx = fnc(x);
...
module.exports.minimize_L_BFGS = function (fnc, grd, x0) {
    ...
    var eps = 1e-8; // max abs value of gradient component for termination
    ...
    while (!convergence ) {
        ...
        if (vect_max_abs_x_less_eps(gn, eps) && fnc(x)) {
            break;
        }
